...
export function KoaRestful(options?: KoaRestfulOptions) {
  ...
  for (const controller of controllers) {
    ...
    const methods: RecordMethods = Reflect.getMetadata(
      KEY_METHOD,
      controller.cls
    );
    for (const property in methods) {
      ...
      arr.forEach(({ route, propertyKey }) => {
        const parameters: Record<number, ParameterConverterFn> =
          Reflect.getMetadata(
            `${KEY_PARAMETER}:${propertyKey}`,
            controller.cls
          ) || {};
        const path = join(
          prefix,
          isNullOrUndefined(route) ? propertyKey : (route as string)
        );
        log?.(`${property} ${path}`);
        router[property as RequestMethod](
          join(path),
