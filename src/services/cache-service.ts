...
interface ICacheService {
     ...
     */
    get<T = any>(key: string, create?: () => Promise<T>, cachingStrategy?: ObjectCachingStrategy): Promise<T | undefined>
    /**
     * 缓存对象是否存在
     * @param key 缓存KEY
     */
     ...
     * @param cachingStrategy 对象缓存策略
     */
    set<T = any>(key: string, value: T, cachingStrategy?: ObjectCachingStrategy): Promise<void>
    /**
     * 获取缓存Key数组
     */
...
abstract class CacheService implements ICacheService {
    ...
    abstract get<T = any>(key: string, create?: (() => Promise<T>) | undefined, cachingStrategy?: ObjectCachingStrategy | undefined): Promise<T | undefined>
    abstract has(key: string): Promise<boolean>
    abstract set<T = any>(key: string, value: T, cachingStrategy?: ObjectCachingStrategy | undefined): Promise<void>
...
class ObjectCaching {
     ...
     */
    constructor(value: any, type: 'absolute' | 'sliding' = 'sliding', expired: number = 5 * 60) {
        this._value = value
        this._type = type
    ...
    value() {
        ...
        if (this._type === 'sliding') {
            this._expirationTime = this.calculateExpirationTime(this._expirationTime, this._expired)
        }
        return this._value
    }
    /**
...
class MemoryCacheService extends CacheService {
    ...
    async get<T = any>(key: string, create?: (() => Promise<T>) | undefined, cachingStrategy?: ObjectCachingStrategy | undefined): Promise<T | undefined> {
        ...
        if (!cache && create) {
            ...
            this._cache[key] = cache = new ObjectCaching(value, strategy.type, strategy.expired)
        }
        return cache?.value()
    }
    ...
    async set<T = any>(key: string, value: T, cachingStrategy?: ObjectCachingStrategy | undefined): Promise<void> {
        const strategy: ObjectCachingStrategy = {
            type: 'sliding',
            expired: 5 * 60,
            ...cachingStrategy,
        }
        this._cache[key] = new ObjectCaching(value, strategy.type, strategy.expired)
    }
